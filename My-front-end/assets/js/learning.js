...
startHeartbeat: function(lessonId, type = 'youtube') {
    ...
    try {
        ...
        const res = await $.ajax({
            url: 'https://localhost:7106/api/comment',
            type: 'POST',
            contentType: 'application/json',
            headers: {
   ...
   loadComments: async function (lessonId) {
    ...
    try {
        const res = await $.ajax({
            url: `https://localhost:7106/api/comment/lesson/${lessonId}`,
            type: 'GET',
...
saveEdit: async function(id) {
    ...
    try {
        const token = localStorage.getItem("jwt_token");
        await $.ajax({
            url: `https://localhost:7106/api/comment/update/${id}`,
            type: 'PUT',
            contentType: 'application/json',
            headers: { 'Authorization': `Bearer ${token}` },
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            try {
                ...
                const token = localStorage.getItem("jwt_token");
                const res = await $.ajax({
                    url: `https://localhost:7106/api/comment/delete/${id}`,
...
handleReaction: async function(commentId, type, btn) {
    ...
    try {
        ...
        const res = await $.ajax({
            url: `https://localhost:7106/api/comment/handleLike/${commentId}`,
            type: 'POST',
            contentType: 'application/json',
            headers: { 'Authorization': `Bearer ${token}` },
...
showReactionDetails: async function(commentId) {
    const $modal = $('#reactionModal');
        ...
        try {
            const res = await $.get(`https://localhost:7106/api/comment/getReactions/${commentId}`);
